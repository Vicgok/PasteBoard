...
const OptimizedActiveClipBoard: React.FC<ActiveClipBoardProps> = React.memo(
  () => {
    ...
    // Memoized content types configuration
    ...
    // Initialize clipboard service and setup real-time subscription
    useEffect(() => {
      if (!user?.id) return;
    ...
    // Auto-detect content type - memoized for performance
    const detectContentType = useCallback(
      (content: string): "text" | "code" | "url" | "other" => {
    ...
    useEffect(() => {
      if (activeContent) {
        const detectedType = detectContentType(activeContent);
        setSelectedType(detectedType);
      }
    }, [activeContent, detectContentType, setSelectedType]);

    // Optimized copy handler with debouncing
    ...
    }, [activeContent, selectedType, user?.id]);
    ...
    // Optimized search dialog handlers
    ...
    const handleSearchItemClick = useCallback(
      ...
      [setActiveContent, setSelectedType, handleSearchClose]
    );

    // Memoized content type selector
    const ContentTypeSelector = useMemo(
      () => (
        ...
        <Card className="shadow-lg rounded-2xl">
          <CardContent className="flex items-center justify-center h-64">
            <p className="text-gray-500">
              Please sign in to use clipboard features
            </p>
          </CardContent>
        </Card>
      );
          ...
          <CardContent className="flex flex-col space-y-6 h-full">
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                onClick={handleCopy}
                disabled={!activeContent.trim() || !user || loading}
              >
                {copySuccess ? (
...
OptimizedActiveClipBoard.displayName = "OptimizedActiveClipBoard";

export default OptimizedActiveClipBoard;
