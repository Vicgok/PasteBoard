// components/optimized/RecentHistory.tsx
...
import { Pencil, Trash2, Clipboard, RefreshCw, X, Search } from "lucide-react";
...
const RecentHistory: React.FC<RecentHistoryProps> = React.memo(
  ({ maxItems = 5 }) => {
    const { user } = useAuthStore();
    const { items, loading } = useClipboardStore();
    ...
    const [editType, setEditType] = useState<ClipboardType>("text");

    // Memoized display items (first n items)
    ...
    // Fetch history when user changes or refresh is triggered
    useEffect(() => {
      if (user?.id) {
    ...
    const getColorScheme = useCallback((type: string) => {
    ...
    const getDisplayType = useCallback((item: ClipboardItem) => {
    ...
    const handleHistoryItemClick = useCallback(async (item: ClipboardItem) => {
      try {
    ...
    const handleRefresh = useCallback(() => {
      if (user?.id) {
        clipboardService.fetchClipboardHistory(user.id, 50, true);
      }
    }, [user?.id]);

    const handleEditClick = useCallback((item: ClipboardItem) => {
      setEditItem(item);
    ...
    const handleSaveEdit = useCallback(async () => {
      ...
      try {
        await clipboardService.updateClipboardEntry(editItem.id, {
          content: editContent,
    ...
    }, [editItem, editContent, editType]);

    const handleDelete = useCallback(async (id: string) => {
      try {
        await clipboardService.deleteClipboardEntry(id);
    ...
    return (
      <>
        ...
        <Dialog open={viewAllOpen} onOpenChange={setViewAllOpen}>
          <DialogContent className="w-full max-w-3xl h-[80vh] flex flex-col p-0 [&>button]:hidden">
            {/* Header */}
            <DialogHeader className="flex flex-row items-center justify-between border-b px-6 py-4">
              <DialogTitle className="text-lg">Clipboard History</DialogTitle>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="flex-1 flex flex-col p-6 space-y-4">
              ...
              <div className="flex-1 overflow-y-auto space-y-3">
                {filteredItems.map((item) => (
                  <div
                    key={item.id}
                    className="p-3 border rounded-lg bg-white flex items-center justify-between hover:shadow transition-colors w-full"
                  >
                    <div className="flex-1 min-w-0">
                      <Badge className="text-xs mb-1" variant="secondary">
                        {getDisplayType(item)}
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                {(["text", "code", "url", "other"] as const).map((type) => (
                  <button
                    key={type}
